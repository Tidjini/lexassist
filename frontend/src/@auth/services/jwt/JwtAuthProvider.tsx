...
function JwtAuthProvider(props: FuseAuthProviderComponentProps) {
	...
	const { setValue: setRefreshTokenValue, removeValue: removeRefreshTokenValue } =
		useLocalStorage<string>('jwt_refresh_token');
	...
	const signIn: JwtAuthContextType['signIn'] = useCallback(
		...
		[setTokenStorageValue, setRefreshTokenValue]
	);
	...
	const signOut: JwtAuthContextType['signOut'] = useCallback(() => {
		...
		removeRefreshTokenValue();
		removeGlobalHeaders(['Authorization']);
		// Vide le cache React Query : sans ça, les données du compte précédent
		// (ex: liste des professeurs, réservée à la gestion) peuvent rester en
		// mémoire et perturber le rendu si un autre compte se connecte ensuite
		// dans le même onglet, sans rechargement complet de la page.
		queryClient.clear();
		setAuthState({
			authStatus: 'unauthenticated',
			isAuthenticated: false,
